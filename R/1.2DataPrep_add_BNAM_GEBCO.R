#removed ME_NH,and MA because unlilke the other 3, 
...
catch_data <- read_rds(here("Data/Derived/all_raw_halibut_catch_formattedAl14.rds"))
...
write.csv(catch_df, here("Data/Derived/all_raw_halibut_catch_with_st_June19.csv"))
...
catch_data_save<-catch_data
catch_df <- as.data.frame(catch_data)
write.csv(catch_df, here("Data/Derived/all_raw_halibut_catch_with_st_Bt_June19.csv"))

#ADD DEPTH----GEBCO Compilation Group (2024) GEBCO 2024 Grid (doi:10.5285/1c44ce99-0a0d-5f4f-e063-7086abc0ea0f)
Depth_raster <- raster("C:/Users/fergusonk/Documents/Shapefiles/Bathymetry/GEBCO_19_Jun_2025_4792f17f10dd/GEBCO_19_Jun_2025_4792f17f10dd/gebco_2024.tif")

#match the CRS of the GEBCO raster to the catch data
crs(catch_data)
crs(Depth_raster)
#Depth_raster <- projectRaster(Depth_raster, crs = crs(catch_data))
...
#make sure everything was populated
catch_df2 <- catch_df %>%
...
names(catch_df2)
write.csv(catch_df2, here("Data/Derived/all_raw_halibut_catch_with_covariates_June19.csv"))
library(ggplot2)
library(dplyr)


#Remove outliers and scale covariates (mean of 0 and standard deviation of 1)
Data = read.csv(here::here("Data/Derived/all_raw_halibut_catch_with_covariates_June19.csv"))
#identify & remove covariate outliers
head(Data)
#	Plot How many tows occur at each depth and how much are they catching there per survey 
# summary counts per 25 m bin, per SURVEY
...
binned <- Data %>%
  mutate(
    Depth_bin = cut(
      Depth,
      breaks = seq(0, ceiling(max(Depth, na.rm = TRUE) / 50) * 50, by = 25),
      include.lowest = TRUE,
      right = FALSE
    )
  ) %>%
  group_by(SURVEY, Depth_bin) %>%
  summarise(
    total = n(),
    pres = sum(PRESENCE == 1, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  mutate(
    Depth_mid = as.numeric(sub("^\\[(.*),(.*)\\)$", "\\1", Depth_bin)) + 12.5
  )

#there are some random outliers at depths so remove if the total is 4 or less
binned2 <- binned %>% filter(total > 3)

#plot
ggplot(binned2, aes(x = Depth_mid)) +
  geom_col(aes(y = total), fill = "darkorange") +
  geom_line(data = binned2 %>% filter(pres > 0),
            aes(y = pres),
            color = "steelblue", size = 1) +
  geom_point(data = binned2 %>% filter(pres > 0),
             aes(y = pres),
             color = "darkblue", size = 2) +

  facet_grid(~SURVEY, scales = "free_x", space = "free_x") +
  geom_text(aes(y = total, label = total),
            vjust = -0.3, size = 3)+
  labs(
    x = "Depth (m)",
    y = "Count(bars = Tows, blue = PRESENCE)",
    title = "Count of Tows and Presence per 25m Depth Bin by Survey"
  ) +
  theme_bw()

depth_range_by_survey <- Data %>%
  group_by(SURVEY) %>%
  summarise(
...
Data %>%
  ...
  count()#n637
Data %>%
  filter(BT_monthly > 18) %>%
  count()#n22

Data_clean <- Data %>%
  filter(Depth > -1000 & Depth < -30 ,  BT_monthly < 18) # 60116-56335=3781dropped
...
write.csv(Data_scaled, here("Data/Derived/Halibut_Catch_Covariates_Scaled_June19.csv"))
