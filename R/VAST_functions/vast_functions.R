...
vast_build_sdm <- function(settings, spatial_list = NULL, extrapolation_list = NULL, extrap_grid = NULL, sample_data, covariate_data, X1_formula, X2_formula, X_contrasts, Xconfig_list, catchability_data, Q1_formula, Q2_formula, index_shapes = NULL, spatial_info_dir = NULL, model_selection) {
  ...
  vast_build_out <- fit_model_aja("settings" = settings, "Method" = settings$Method, "model_selection" = model_selection, "input_grid" = extrap_grid, "Lat_i" = sample_data[, "Lat"], "Lon_i" = sample_data[, "Lon"], "t_i" = sample_data[, "Year"], "c_i" = rep(0, nrow(sample_data)), "b_i" = sample_data[, "Biomass"], "a_i" = sample_data[, "Swept"], "PredTF_i" = sample_data[, "Pred_TF"], "X1config_cp" = Xconfig_list[["X1config_cp"]], "X2config_cp" = Xconfig_list[["X2config_cp"]], "covariate_data" = covariate_data, "X1_formula" = X1_formula, "X2_formula" = X2_formula, "X_contrasts" = X_contrasts, "catchability_data" = catchability_data, "Q1_formula" = Q1_formula, "Q2_formula" = Q2_formula, "Q1config_k" = Xconfig_list[["Q1config_k"]], "Q2config_k" = Xconfig_list[["Q2config_k"]], "newtonsteps" = 1, "getsd" = TRUE, "getReportCovariance" = TRUE, "run_model" = FALSE, "test_fit" = FALSE, "Use_REML" = FALSE, "getJointPrecision" = TRUE, "index_shapes" = index_shapes, "DirPath" = spatial_info_dir)
...
vast_fit_sdm <- function(vast_build_adjust, model_selection = model_selection, nice_category_names, index_shapes = NULL, spatial_info_dir = NULL, run_final_model = run_final_model, out_dir) {
