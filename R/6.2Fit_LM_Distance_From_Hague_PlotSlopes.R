...
#dist_hague_CA<-read.csv(here::here("2024-10-04/Output/Shift_Indicators/dist_hague_CA_seasonal.csv"))
...
#3 regional plots for DFH parameters
#Overall
...
Overall.DFHfiltered_df <- Overall.DFHcoefficients_df[Overall.DFHcoefficients_df$Season=="Spring",]%>%
  ...
  filter(term == "Year")  # Replace "x" with "Intercept" to plot intercept
Overall.DFHfiltered_df$Region<-factor(Overall.DFHfiltered_df$Region,levels=c("USA","Canada"))
#----

ggplot(Overall.DFHfiltered_df  , aes(x = factor(Region), y = estimate,fill=Period)) +
  geom_errorbar(aes(ymin = conf.low, ymax =conf.high, , color = factor(Region)),position=pd, , show.legend = FALSE)+
  geom_point(shape=21, size = 3,position=pd) +
  #scale_y_reverse()+
  coord_flip()+
      scale_fill_manual(values=c("steelblue", "orangered"))+
  scale_color_manual(values = c("Canada" = "orange", "USA" = "darkblue")) + # Define colors for each region
...
dist_hague_Reg<-read.csv(here::here("2024-10-04/Output/Shift_Indicators/dist_hague_Reg_seasonal.csv"))


dist_hague_RegFromZero<-dist_hague_Reg %>%
  mutate(
    Dist_Mean = case_when(
      Region %in% c("USA") ~ Dist_Mean * -1,
      TRUE ~ Dist_Mean
    ),
    Dist_Med = case_when(
      Region %in% c("USA") ~ Dist_Med * -1,
      TRUE ~ Dist_Med
    ),
    Dist_Q5 = case_when(
      Region %in% c("USA") ~ Dist_Q5 * -1,
      TRUE ~ Dist_Q5
    ),
    Dist_Q95 = case_when(
      Region %in% c("USA") ~ Dist_Q95 * -1,
      TRUE ~ Dist_Q95
    )
  )
...
DFHtimeseries <- ggplot(dist_hague_RegFromZero_Spring, aes(x = Year, y = Dist_Mean, color = Region)) +
...
Canada <- subset(spring_dist_hague_Reg, Region == "Canada")
USA <- subset(spring_dist_hague_Reg, Region == "USA")
