...
fill_na_spatiotemporal <- function(na_pts, non_na_pts, value_col, date_col) {
  if (FALSE) {
    na_pts = na_points
    na_pts = na_pts |> filter(EST_YEAR >= 1993)
    non_na_pts = non_na_points
    value_col = names(summ_df_list)[j]
    date_col = date_col_name

    na_pts <- na_pts_filled[is.na(na_pts_filled$BT_seasonal), ]
    na_pts_filled_good<- na_pts_filled
  }
  
  # Results
  na_pts_filled<- na_pts
  ...
  if(is.null(date_col)) {
    for (i in 1:nrow(na_pts)) {
      ...
      na_pts_filled[[value_col]][i] <- non_na_pts[[value_col]][nearest_idx]
    }
  }
  ...
  if(!is.null(date_col)){
     for (i in 1:nrow(na_pts)) {
      ...
      non_na_pts_sub <- non_na_pts |>
        dplyr::filter(get({{ date_col }}) == current_date)
      
      if(nrow(non_na_pts_sub) == 0){
        non_na_pts_sub_a <- non_na_pts |>
          dplyr::filter(get({{ date_col }}) + 1 == current_date)
        non_na_pts_sub_b <- non_na_pts |>
          dplyr::filter(get({{ date_col }}) - 1 == current_date)
        non_na_pts_sub <- bind_rows(non_na_pts_sub_a, non_na_pts_sub_b)
      }
      
      if(!all(is.na(non_na_pts_sub))){
        # Distances
        ...
        # Fill NA with nearest neighbor value
        na_pts_filled[[value_col]][i] <- non_na_pts_sub[[value_col]][nearest_idx]
        # print(na_pts_filled[[value_col]][i])
      } else {
        na_pts_filled[[value_col]][i] <- NA
      }
    } 
  }
  return(na_pts_filled)
}
...
dynamic_2d_extract<- function(rast_ts_stack, stack_name, t_summ, t_position, interp_nas, sf_points, date_col_name, df_sf){
  ...
  if(interp_nas){
    ...
    filled_points <- fill_na_spatiotemporal(na_pts = na_points, non_na_pts = non_na_points, value_col = names(summ_df_list)[j], date_col = date_col_name)
...
dynamic_2d_extract_wrapper<- function(dynamic_covariates_list, t_summ, t_position, interp_nas, sf_points, date_col_name, df_sf, out_dir){
  ...
  if(FALSE){
    ...
    dynamic_covariates_list = dynamic_stacks
    t_summ = "seasonal"
    t_position = NULL
    interp_nas = TRUE
    sf_points = all_tows_with_static_covs
